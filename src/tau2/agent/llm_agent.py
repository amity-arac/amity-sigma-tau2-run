...
Please be carefully to avoid the mistake which another agent can make before, please see an example in <mistake_avoiding>
""".strip()

MISTAKE_AVOIDING = """
## Please Take Attention on this sections because many agent might make a mistake, don't make a mistake on this again

### Section 1: Key Failure Patterns

| Pattern | Severity | Description |
|---------|----------|----------|----------|-------------|
| **Wrong Order Selection** | Critical | Agent operates on wrong order when user has multiple orders |
| **Wrong Item Selection** | Critical | Agent selects wrong/partial item_ids from an order |
| **Wrong Variant Selection** | High | Agent picks wrong replacement variant (new_item_ids) |
| **Mutation Not Called** | High | Agent claims success without executing mutation tool |
| **Incomplete Multi-Task** | Moderate | Agent completes some tasks but forgets others |
| **Wrong Payment Method** | Moderate | Agent uses incorrect payment method |
| **Conditional Flow Handling** | Moderate | Agent fails when user changes their mind mid-conversation |
...
|----------|------|----------------|
| **P0** | Order Disambiguation | Always list ALL matching orders with full details (order_id, items, status, address). Require explicit user confirmation of order_id before any mutation. |
| **P0** | Multi-Item Confirmation | After user specifies items, echo back the COMPLETE list: "I will [action] these items: [list]. Is this correct?" |
| **P0** | Execution Verification | Never claim success until mutation tool returns. Add explicit logging: "Executing [tool_name] now..." before calling. |
| **P1** | Same-Item Replacement | When user says "same item" or "replacement", use the EXACT same item_id. Only change variant if user explicitly requests different specs. |
| **P1** | Payment Method Flow | Always enumerate payment options and require explicit user selection. Never assume default payment method. |
| **P1** | Task Tracking | Maintain explicit checklist of pending tasks. After each action, enumerate remaining tasks. Don't close conversation until all tasks confirmed complete. |
| **P2** | State Change Detection | When user says "wait", "actually", "on second thought", pause and re-verify the entire action plan before proceeding. |
| **P2** | Cross-Verification | Before mutation, verify: order_id matches user's description, item_ids exist in that order, new_item_ids match user's constraints. |
"""
...
class LLMAgent(LocalAgent[LLMAgentState]):
    ...
    def system_prompt(self) -> str:
        return SYSTEM_PROMPT.format(
            domain_policy=self.domain_policy, agent_instruction=AGENT_INSTRUCTION, mistake_avoiding=MISTAKE_AVOIDING
        )
